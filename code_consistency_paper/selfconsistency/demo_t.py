...
def run_vote_no_threads(image, solver, exif_to_use, n_anchors=1, num_per_dim=None,
    ...
    # define patch size and num per dim
    ...
    for anchor_idx in range(n_anchors):
        ...
        for i in np.linspace(0, h - patch_size, num_per_dim).astype(int):
            for j in np.linspace(0, w - patch_size, num_per_dim).astype(int):
                ...
                #print("batch_b: ", np.array(batch_b).shape)
        ...
        if len(batch_b) > 0:
            ...
            #print("prev_batch: ", np.array(prev_batch).shape)
            if not prev_batch:
                to_pad = np.zeros((64,128,128,3))[:batch_size - batch_b_len]
...
def rect_over(area1, area2):
    #print(area1, area2)
    return area1[0] < area2[2] and area1[1] < area2[3] and area1[2] > area2[0] and area1[3] > area2[1]

class Demo_t():
    def __init__(self, ckpt_path='/data/scratch/minyoungg/ckpt/exif_medifor/exif_medifor.ckpt', use_gpu=0,
                 quality=3.0, patch_size=128, num_per_dim=30, obj_area=None):
        ...
        self.bu = benchmark_utils.EfficientBenchmark(self.solver, nc, params, im, auto_close_sess=False,
                                                     ...
                                                     patch_size=patch_size, num_per_dim=num_per_dim, obj_area=obj_area,
                                                     use_area=True)
        self.obj_area = obj_area
        return

    def set_obj_area(self, obj_area):
        self.obj_area = obj_area

    def run(self, im, gt=None, show=False, save=False,
            blue_high=False, use_ncuts=False, use_area=False):
        # run for every new image
        if use_area:
            self.bu.reset_image(im, use_area=True)
        else:
    ...
    def run_vote(self, im, num_per_dim=3, patch_size=128):
        ...
        h, w = np.shape(im)[:2]
        all_results = []
        hstep = round((h-patch_size) / (num_per_dim-1+0.0000001))
        wstep = round((w-patch_size) / (num_per_dim-1+0.0000001))
        for hSt in np.linspace(0, h - patch_size, num_per_dim).astype(int):
            for wSt in np.linspace(0, w - patch_size, num_per_dim).astype(int):
                #print(hSt, wSt)
                f = False
                for area in self.obj_area:
                    area2 = [wSt, hSt, wSt+wstep, hSt+hstep]
                    if rect_over(area, area2):
                        f = True
                        break
                if f:
                    #print("patch covered")
                    ...
                    res = run_vote_no_threads(im, self.solver, None, n_anchors=1, sample_ratio=self.quality,
                                              override_anchor=(hSt, wSt))['out']['responses'][0]
                    all_results.append(res)
        self.all_result = all_results
        return dbscan_consensus(all_results)

    def __call__(self, url, dense=False, use_area=False):
        """
        @Args
        ...
        if not dense:
            ...
            # Runs DBSCAN
            out = self.run(im, use_area=use_area)
        return im, out
...
if __name__ == '__main__':
    ...
    exif_demo = Demo_t(ckpt_path=ckpt_path, use_gpu=0, quality=3.0, num_per_dim=30, obj_area=face_data)
